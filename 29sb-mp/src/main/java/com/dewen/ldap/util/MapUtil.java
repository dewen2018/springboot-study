...
public class MapUtil {
     ...
     * 
     * @param map
     * @param property keys表示获取key的集合，返回Collection<Object>,values表示获取value集合，返回Collection<Object>，sizes表示获取大小，返回数字
     * @return
     */
    public static Object getProperty(Map<String, Object> map, String property) {
        ...
        if ("keys".equals(property)) {
            return new ArrayList<String>(map.keySet());
        } else if ("values".equals(property)) {
            return map.values();
        } else if ("sizes".equals(property)) {
            return map.size();
        }
        return null;
    }
