...
import com.dewen.exception.BusinessException;
...
public class BackendAService implements Service {
    ...
    public Mono<String> monoSuccess() throws InterruptedException {
        // Thread.sleep(5000);
        // return Mono.just("Hello World from backend A");
        WebClient client = builder.build();
        Mono<String> response = client.get()
                .uri("http://dewen.com:8082/node2/backendA/success")
                .retrieve()
    ...
    private Mono<String> monoFallback(RequestNotPermitted ex) {
        return Mono.just("Recovered: " + ex.toString());
    }

    public Mono<String> monoFallback(Throwable throwable) {
        return Mono.just("Recovered: " + throwable.getMessage());
    }
    ...
    // private Mono<String> monoFallback(String param1, BulkheadFullException ex) {
    //     return Mono.just("Handled the exception when the Bulkhead is full");
    // }
    //
    // private Mono<String> monoFallback(String param1, NumberFormatException ex) {
    //     return Mono.just("Handled the NumberFormatException");
    // }
    //
    // private Mono<String> monoFallback(String param1, Exception e) {
